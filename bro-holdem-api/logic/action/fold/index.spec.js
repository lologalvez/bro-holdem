...
describe('logic - action - fold', () => {
    ...
    it('should succeed on correct data', async () => {
        const result = await logic.fold(gameId, twoId)
        ...
        expect(callPlayer.inHand).to.equal(false)
        expect(callPlayer.cards.length).to.equal(0)
        expect(lastAction).to.exist
        expect(lastAction[0].type).to.equal('fold')
