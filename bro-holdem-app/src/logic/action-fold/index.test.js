...
describe.only('logic - action fold', () => {
    ...
    it('should succeed on correct data', async () => {
        const response = await logic.actionFold(gameId)
        ...
        expect(callPlayer.inHand).toBe(false)
        expect(lastAction).toBeDefined()
        expect(lastAction[0].type).toBe('fold')
    ...
    it('should fail on incorrect gameId', async () => {
        ...
        try {
            await logic.actionFold(_gameId)
        } catch (error) {
            expect(error).toBeDefined()
